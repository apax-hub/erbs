...
from erbs.bias import MetaDCutFactory, OPESExploreFactory
...
def run_traj(atoms, calc, ts, T, max_steps):
    ...
    dyn = NVTBerendsen(atoms, ts * units.fs, temperature_K=T, taut=0.005*1000*units.fs)
    ...
    for i in trange(1, max_steps+1, leave=False):
        ...
        labeled_atoms.calc = SinglePointCalculator(labeled_atoms, energy_biased= calc.results["energy"], **calc.base_results)
